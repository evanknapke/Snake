...
public class Snake implements ActionListener, KeyListener {
    ...
    public JFrame jframe; // everything placed into
    ...
    public static final int UP = 0, DOWN = 1, LEFT = 2, RIGHT = 3, SCALE = 10; // final means it will never change (CAPS)
    ...
    public void actionPerformed(ActionEvent arg0) {
        ...
        if (ticks % 2 == 0 && head != null && !over && !paused) { // every 10 seconds
            time++;
            snakeParts.add(new Point(head.x, head.y));
            ...
            if (direction == DOWN) {
                if (head.y + 1 < 67 && noTailAt(head.x, head.y + 1)) { //head.y + 1 < dim.height / SCALE
                    head = new Point(head.x, head.y + 1);
                } else {
                    over = true;
                }
            ...
            if (direction == RIGHT) {
                if (head.x + 1 < 80 && noTailAt(head.x + 1, head.y)) { //head.x + 1 < dim.width / SCALE
                    head = new Point(head.x + 1, head.y);
                } else {
                    over = true;
                }
